...
/obj/machinery/computer/vox_station/proc/vox_move_to(area/destination as area)
	...
	if(announce_vox_departure)
		if(curr_location == locate(/area/shuttle/vox/station))
			command_announcement.Announce("Attention, Hypatia, we just tracked a small target bypassing our defensive perimeter. Can't fire on it without hitting the station - you've got incoming visitors, like it or not.", "NSV Icarus")
		else if(dest_location == locate(/area/shuttle/vox/station))
			command_announcement.Announce("Your guests are pulling away, Hypatia - moving too fast for us to draw a bead on them. Looks like they're heading out of Tau Ceti at a rapid clip.", "NSV Icarus")
