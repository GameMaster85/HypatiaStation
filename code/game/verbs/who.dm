...
/client/verb/staffwho()
	...
	if(holder)
		for(var/client/C in admins)
			if(R_ADMIN & C.holder.rights || (!R_DONOR & C.holder.rights && !R_MOD & C.holder.rights && !R_MENTOR & C.holder.rights))	//Used to determine who shows up in admin rows
		...
		for(var/client/C in admins)
			if(R_ADMIN & C.holder.rights || (!R_DONOR & C.holder.rights && !R_MOD & C.holder.rights && !R_MENTOR & C.holder.rights))
				if(!C.holder.fakekey)
					msg += "\t[C] is a [C.holder.rank]\n"
					num_admins_online++
